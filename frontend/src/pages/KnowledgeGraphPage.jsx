...
function initKnowledgeGraphRuntime() {
  let cleanup;
  let cancelled = false;

  import('./knowledgeGraphRuntime.js').then(async ({ initKnowledgeGraphRuntime: initRuntime }) => {
...
function KnowledgeGraphPageMarkup() {
  return (
    <>
...
<button className="text-gray-400 hover:text-white transition-colors relative"><i className="fa fa-bell text-lg"></i><span className="absolute -top-1 -right-1 w-2 h-2 bg-red-500 rounded-full animate-pulse"></span></button>
...
<a href="#" className="block px-4 py-2 text-sm text-red-400 hover:bg-white/5 transition-colors" data-logout-link><i className="fa fa-sign-out mr-2"></i>退出登录</a>
</div>
</div>
...
export default function KnowledgeGraphPage({ page }) {
  useLegacyPageRuntime(page, initKnowledgeGraphRuntime);

  return <KnowledgeGraphPageMarkup />;
}
