...
export async function initKnowledgeGraphRuntime() {
...
function initNetwork() {
  nodesDataSet = new DataSet(allNodes.map(node => ({
    id: node.id,
    label: node.label,
    group: node.group,
    color: {
  ...
  edgesDataSet = new DataSet(allEdges.map(edge => ({
    id: edge.id,
    from: edge.from,
    to: edge.to,
    label: '',
  ...
  network = new Network($('knowledgeGraph'), { nodes: nodesDataSet, edges: edgesDataSet }, {
    nodes: {
      chosen: {
